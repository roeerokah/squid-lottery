...
var Deck = (function () {
  ...
  function _card(i) {
    ...
    // self = card
    var self = { i: i, rank: rank, suit: suit, pos: i, $el: $el, mount: mount, unmount: unmount, setSide: setSide, cardWidth: cardWidth, cardHeight: cardHeight };
  ...
  var poker = {
    deck: function deck(_deck4) {
      ...
      function poker(next) {
        ...
        var started = false;
        __fontSize = fontSize();
        var xOffset = cards[0].$el.getBoundingClientRect().left - 10;
        var yOffset = cards[0].$el.getBoundingClientRect().top - 10;
        var isCompleted = false;
        cards.forEach(function (card, i) {
          if(i % _deck4.itemsPerRow === 0) {
            if (started) {
              j = j + 1;
            }
            started = true;
          ...
          card.poker(i % _deck4.itemsPerRow,j,xOffset, yOffset, len, function (i) {
            card.setSide('back');
            if (i === _deck4.itemsPerRow - 1 && !isCompleted) {
              isCompleted = true;
              _deck4.trigger('complete');
              next();
            }
          });
        });
    ...
    card: function card(_card4) {
      ...
      _card4.poker = function (i,j,xOffset, yOffset, len, cb) {
        var delay = i * 250;
        _card4.animateTo({
          delay: delay,
          duration: 250,
          x: (xOffset - 10 - i*_card4.cardWidth )* -1 , //Math.round((i - 2.05) * 70 * __fontSize / 16),
          y: (yOffset -10 - j*_card4.cardHeight*1.01)* -1, // Math.round(-110 * __fontSize / 16),
          rot: 0,
  ...
  function Deck(cardsNumber) {
    // init cards array
    var cards = new Array( cardsNumber);
    var squidPerRow = Math.ceil(Math.sqrt(cards.length));
    var $el = createElement('div');
    var self = observable({ mount: mount, unmount: unmount, setCards : setCards, cards: cards, $el: $el, itemsPerRow: squidPerRow });
    var $root;
    ...
    // create cards
    setCards(cardsNumber);

    return self;

    function setCards(cardsNumber) {
      cards = new Array( cardsNumber);
      squidPerRow = Math.ceil(Math.sqrt(cards.length));
      self.cards = cards;
      self.itemsPerRow = squidPerRow;
      for (let i = cards.length; i; i--) {
        card = cards[i - 1] = _card(i - 1);
        card.cardWidth = (window.innerWidth / squidPerRow) - 5;
        card.cardHeight = (window.innerHeight / squidPerRow) - 5;
        card.setSide('back');
        card.mount($el);
      }
    }
